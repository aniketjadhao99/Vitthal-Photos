  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Assuming newest are at the end, or filtered by a New category
        // Here we'll just reverse to show latest if no specific 'New' category exists
        const newProducts = allProducts.filter(p => p.category === 'New').length > 0 
                            ? allProducts.filter(p => p.category === 'New')
  ...
  return (
    <>
      <div className="breadcrumbs" style={{ padding: '20px 20px', maxWidth: '1200px', margin: '0 auto' }}>
        <Link to="/">Home &gt;</Link> New Arrivals
      </div>

      <div className="category-header" style={{ textAlign: 'center', padding: '40px 20px', background: 'linear-gradient(135deg, #f9f0ff 0%, #fff 100%)', marginBottom: '40px' }}>
      ...
      <div className="collection-container" style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 20px', marginBottom: '80px' }}>
        {loading ? (
              ...
              <div className="T-product" key={product._id}>
                ...
                <div className="Trending-product-details">
                  ...
                  <div className="T-product-span">
                    <span className="text-[#8a7560] text-sm">{product.category} Collection</span>
          ...
          <div style={{ textAlign: 'center', padding: '50px' }}>No new products found right now. Check back later!</div>
        )}
      </div>
    </>
  );
