  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Filter by Warrior category
        setProducts(allProducts.filter(p => p.category === 'Warriors' || p.category === 'Warrior'));
      } catch (err) {
        console.error(err);
      } finally {
        setLoading(false);
  ...
  return (
    <>
      <div className="breadcrumbs" style={{ padding: '20px 20px', maxWidth: '1200px', margin: '0 auto' }}>
        <Link to="/">Home &gt;</Link> Warrior Frames
      </div>

      <div className="category-header" style={{ textAlign: 'center', padding: '40px 20px', background: 'linear-gradient(135deg, #f0f5ff 0%, #fff 100%)', marginBottom: '40px' }}>
        <h1 style={{ fontSize: '2.5rem', color: '#333', marginBottom: '10px', fontWeight: 800 }}>Warrior Collection</h1>
        <p style={{ color: '#666', fontSize: '1.1rem', maxWidth: '600px', margin: '0 auto' }}>Celebrate the legacy and valor of India's greatest heroes with our premium warrior frames.</p>
      </div>
              ...
              <div className="T-product" key={product._id}>
                ...
                <div className="Trending-product-details">
                  ...
                  <div className="T-product-span">
                    <span className="text-[#8a7560] text-sm">Warrior Collection</span>
