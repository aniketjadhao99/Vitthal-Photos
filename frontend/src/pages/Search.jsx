...
const Search = () => {
  ...
  return (
    <>
      ...
      <div className="category-header" style={{ textAlign: 'center', padding: '40px 20px', background: 'linear-gradient(135deg, #f0f0f0 0%, #fff 100%)', marginBottom: '40px' }}>
        <h1 style={{ fontSize: '2.5rem', color: '#333', marginBottom: '10px', fontWeight: 800 }}>Search Results</h1>
        <p style={{ color: '#666', fontSize: '1.1rem', maxWidth: '600px', margin: '0 auto' }}>
          {query ? `Found ${products.length} results for "${query}"` : 'Enter a search term to find products.'}
        </p>
      </div>

      <div className="collection-container" style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 20px', marginBottom: '80px' }}>
        {loading ? (
          <div style={{ textAlign: 'center', padding: '50px' }}>Searching...</div>
        ) : products.length > 0 ? (
          ...
          <div style={{ textAlign: 'center', padding: '50px' }}>No products found matching your search.</div>
        )}
      </div>
    </>
  );
