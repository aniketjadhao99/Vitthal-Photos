  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Filter by Collage category
        setProducts(allProducts.filter(p => p.category === 'Collage' || p.category === 'Collages'));
      } catch (err) {
        console.error(err);
      } finally {
        setLoading(false);
  ...
  return (
    <>
      <div className="breadcrumbs" style={{ padding: '20px 20px', maxWidth: '1200px', margin: '0 auto' }}>
        <Link to="/">Home &gt;</Link> Collage Frames
      </div>
      ...
      <div className="category-header" style={{ textAlign: 'center', padding: '40px 20px', background: 'linear-gradient(135deg, #f4ebd9 0%, #fbf8f3 100%)', marginBottom: '40px' }}>
        <h1 style={{ fontSize: '2.5rem', color: '#0b1325', marginBottom: '10px', fontWeight: 800 }}>Collage Collection</h1>
        <p style={{ color: '#141d30', fontSize: '1.1rem', maxWidth: '600px', margin: '0 auto' }}>Beautifully arranged multi-photo frames perfect for creating lasting memories with loved ones.</p>
      </div>
              ...
              <div className="T-product" key={product._id}>
                ...
                <div className="Trending-product-details">
                  ...
                  <div className="T-product-span">
                    <span className="text-[#8a7560] text-sm">Collage Collection</span>
